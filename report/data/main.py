...
data= pd.read_csv('fastext.csv')
X = data[['short_topic','year_text']]
X['label']=X.short_topic.str.replace(' ', '_')
X_train,X_test=train_test_split(X,test_size=.18,random_state=42,shuffle=True)
text=list(X_test.year_text.values)


print('dim=32,epoch=24,minCount=8,wordNgrams=2')
classifier = fastText.train_supervised('data.txt', label='__',dim=32,epoch=24,minCount=8,wordNgrams=2)
print('classifier ок')
